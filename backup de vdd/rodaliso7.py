...
def atualizar_descricao_empurrao(event=None):
    ...
    if preset_selecionado in PRESETS_EMPURRAO:
        ...
        lbl_empurrao_alvo.config(text=f"Arquivo Alvo: {dados['arquivo_alvo']}", fg="#38b6ff")
        lbl_desc_empurrao.config(text=dados["descricao"], fg="#e0e0e0")
...
def aplicar_preset_empurrao():
    ...
    if not preset_selecionado or preset_selecionado not in PRESETS_EMPURRAO:
        lbl_resultado_empurrao.config(text="⚠️ Selecione um preset!", fg="#ffc107")
        return
    ...
    if caminho_destino:
        try:
            ...
            lbl_resultado_empurrao.config(text=f"🔴 ERRO AO GRAVAR PRESET: {e}", fg="#dc3545")
...
tk.Label(col_esq5, text="Selecione o Jogo / Preset:", font=("Arial", 9), fg="#ffffff", bg="#000000").pack(anchor="w")
...
tk.Label(col_dir5, text="DETALHES DO PRESET", font=("Arial", 9, "bold"), fg="#aaaaaa", bg="#121212").pack(pady=(0, 10))

lbl_desc_empurrao = tk.Label(col_dir5, text="Selecione um preset ao lado para ver detalhes e o arquivo alvo.", font=("Arial", 9, "italic"), fg="#aaaaaa", bg="#121212", wraplength=220, justify="left")
lbl_desc_empurrao.pack(anchor="w", pady=(0, 10))

tk.Label(col_dir5, text="PREVIEW DA CONFIGURAÇÃO:", font=("Arial", 8, "bold"), fg="#aaaaaa", bg="#121212").pack(anchor="w", pady=(5, 2))
txt_preview_empurrao = tk.Text(col_dir5, height=8, width=25, font=("Consolas", 8), bg="#1a1a1a", fg="#e0e0e0", insertbackground="white", relief="solid", bd=1)
